...
def _write_ai_files(ws: Path, ai_text: str) -> list[str]:
    ...
    for match in _FILE_BLOCK_RE.finditer(ai_text):
        ...
        # Artifact from empty blocks: the regex captures "===END===" as content
        # when there is nothing between the ===FILE: header and ===END===.
...
async def test_health_check():
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=10) as client:
        for path in ("/health", "/api/health", "/api/v1/health", "/ping"):
...
async def test_api_docs_accessible():
    \"\"\"OpenAPI docs should be publicly accessible (tries common paths).\"\"\"
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=10) as client:
        for path in ("/docs", "/api/docs", "/swagger", "/swagger-ui", "/redoc"):
            resp = await client.get(path)
            if resp.status_code == 200:
                assert resp.status_code == 200
                return
    pytest.skip("No docs endpoint found — docs may be disabled in this environment")


async def test_openapi_schema():
    \"\"\"OpenAPI schema must be valid JSON with the 'openapi' key.\"\"\"
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=10) as client:
        for path in ("/openapi.json", "/api/openapi.json", "/api/v1/openapi.json"):
            resp = await client.get(path)
            if resp.status_code == 200:
                data = resp.json()
                assert "openapi" in data
                return
    pytest.skip("No OpenAPI schema endpoint found")
===END===
""")

...
async def _write_accepted_tests_to_workspace(
    ...
    logger.info(
        "engine: wrote %d Python test files for project %s (%d TS/E2E skipped)",
        written, project_id, skipped,
    )
    return written
...
async def _execute(db: AsyncSession, project_id: str, run_id: str) -> None:
    ...
    if layer == "backend" and is_workspace:
        n = await _write_accepted_tests_to_workspace(db, project_id, run_cwd, run_id)
        if n:
            await _broadcast_run(run_id, {"log": f"[testforge] Loaded {n} accepted test files"})
